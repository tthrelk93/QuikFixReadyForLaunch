                            ...
                            if key as! String == self.enterPromoTF.text{
                                ...
                                if (val as! [String]).contains((Auth.auth().currentUser?.uid)!){
                                   ...
                                   /* if self.jobPost.category1 == "Moving(Home-To-Home)"{
                                    ...
                                    self.calcRate = (self.calcRate - 5)
                                    self.totalFinalCost.text = "$\(String(describing:self.calcRate))/hour + \(self.addRate) service fees"
   ...
   // @IBAction func editPaymentPressed(_ sender: Any) {
    ...
    @IBAction func postJobPressed(_ sender: Any) {
        ...
        if self.cardConnected == true {
            ...
            //values["timeLogs"] = ["studentOnLocation":"false","studentConfirmsArrival":"false","posterConfirmsArrival":"false","studentPressesStart":"false","studentPressesFinish":"false","posterRecievesBill":"false","posterConfirmsBill":"false","posterIsCharged":"false","studentIsPayed":"false"] as! [String:Any]
            
            values["payment"] = "$\(String(describing:calcRate))"
        ...
        values["studentsInRange"] = self.studentsInRange
            ...
            if self.jobPost.category1 == "Tech Help" || self.jobPost.category1 == "Custom"{
                values["customTechDetails"] = self.jobPost.customTechDetails
            }
    ...
    func submitTokenToBackend(token: STPToken, completion: @escaping STPErrorBlock, completionHandler: (Error) -> ()){
        ...
        MyAPIClient.sharedClient.callSaveCard(stripeToken: token, email: self.email, name: self.tempPosterName){ responseObject, error in
           ...
           // values["timeLogs"] = ["studentOnLocation":"false","studentConfirmsArrival":"false","posterConfirmsArrival":"false","studentPressesStart":"false","studentPressesFinish":"false","posterRecievesBill":"false","posterConfirmsBill":"false","posterIsCharged":"false","studentIsPayed":"false"] as! [String:Any]
            ...
            values["workerCount"] = self.jobPost.workerCount
            values["acceptedCount"] = 0
            values["studentsInRange"] = self.studentsInRange
            if self.jobPost.category1 == "Moving(Home-To-Home)"{
    ...
    override func viewDidLoad() {
        ...
        self.calcRate = ((25) * (jobPost.workerCount as! Int))
        self.addRate = 0
        ...
        self.dateLabel.text = "Date 1: \(String(describing: jobPost.date!.first!))"
        self.timeLabel.text = "Time 1: \(String(describing: (jobPost.startTime!.first as! [String]).first!))"
        ...
        } else if jobPost.tools?.first == "1 Truck"{
                    self.totalToolFee.text = "+ $10 Truck Fee"
                    self.addRate = self.addRate + 10
        } else if jobPost.tools?.first == "2 Trucks"{
                    self.totalToolFee.text = "+ $20 Truck Fee"
                    self.addRate = self.addRate + 20
        } else {
            self.totalToolFee.text = "+ $5 Tool Fee"
            self.addRate = self.addRate + 5
        }
        ...
        self.totalFinalCost.text = "$\(String(describing:calcRate))/hour + $\(self.addRate) service fees"
