...
class CreateStudentStep4EnterCardInfoViewController: UIViewController, STPAddCardViewControllerDelegate, STPPaymentCardTextFieldDelegate, STPPaymentMethodsViewControllerDelegate, DataDelegate, WKUIDelegate  {
                ...
                if error != nil {
                    ...
                    storageRef.putData(uploadData!, metadata: nil, completion: { (metadata, error) in
                        ...
                        if let profileImageUrl = metadata?.downloadURL()?.absoluteString {
                            ...
                            self.existingPromoCodes.append(tempPromo)
    ...
    @IBAction func creditPressed(_ sender: Any) {
        ...
        /*uuuuDatabase.database().reference().child("students").child((Auth.auth().currentUser?.uid)!).observeSingleEvent(of: .value, with: { (snapshot) in
   ...
   /* @IBAction func createAccountPressed(_ sender: Any) {
    ...
    override func viewDidLoad() {
        ...
        //uuuupaymentCardTextField.delegate = self
        ...
        //uuuuview.addSubview(paymentCardTextField)
        
        
        Database.database().reference().child("existingPromoCodes").observeSingleEvent(of: .value, with: { (snapshot) in
            if let snapshots = snapshot.children.allObjects as? [DataSnapshot]{
                for snap in snapshots{
                    if let promoString = snap.value as? String{
                        self.existingPromoCodes.append(promoString)
                    }
                }
            }
