...
class studentProfile: UIViewController, UIScrollViewDelegate, UITextViewDelegate, UITextFieldDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, RemoveDelegate, UITabBarDelegate, UITableViewDelegate, UITableViewDataSource, MessagingDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    var skillsData = ["Leaf Blowing", "Gardening", "Gutter Cleaning", "Lawn Care", "Tech Help", "Installations(Electronics)", "Installations(Decorations)", "Furniture Assembly","Moving(In-Home)", "Moving(Home-To-Home)", "Hauling Away", "Custom"]
  ...
  //  @IBOutlet weak var studentBio: UITextView!
    ...
    func loadPageData(){
      ...
      var testCoord = CLLocationCoordinate2D(latitude: 35.101081, longitude:  -89.792666)
        ...
        var testCoord5 = CLLocationCoordinate2D(latitude: 35.09087793812716, longitude:  -89.78937685489655)
       ...
       // print("geoShitAfterRemove: \(locationManager.monitoredRegions)")
            ...
            if let snapshots = snapshot.children.allObjects as? [DataSnapshot]{
                for snap in snapshots{
                    if snap.key == "stripeToken"{
                        if snap.value as! String == "" && self.willConnectStripe != true{
                            self.performSegue(withIdentifier: "ToStripeConnect", sender: self)
                            
                        } else if snap.value as! String == "" && self.willConnectStripe == true{
                            Database.database().reference().child("students").child(Auth.auth().currentUser!.uid).updateChildValues(["stripeToken": self.stripeConnectID])
                        }
                    }
                    if snap.key == "name"{
   ...
   // var job = JobPost()
    ...
    func addGeotificationViewController(didAddCoordinate coordinate: CLLocationCoordinate2D, radius: Double, identifier: String, note: String, eventType: EventType, studentID: String) {
       // controller.dismiss(animated: true, completion: nil)
        ...
        let geotification = Geotification(coordinate: coordinate, radius: clampedRadius, identifier: identifier, note: note, eventType: eventType, studentID: studentID)
...
extension studentProfile: CLLocationManagerDelegate {
