   ...
   var date = String()
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        for date in (self.jobsForDate[indexPath.row].date!){
            if (count + 1) != self.jobsForDate[indexPath.row].date!.count{
                tempString = (tempString + date + ", ")
            } else {
                tempString = (tempString + date)
            }
            count = count + 1
        }
        print("tempStringDAtes: \(tempString)")
        cell.timeLabel.text = "Available Dates: \(tempString)"
        
        cell.additInfoLabel.text = "Info: \(self.jobsForDate[indexPath.row].additInfo!)"
